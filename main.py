...
# CONFIGURATION
# =====================================================================
SOCKET_URL = "wss://socket.india.delta.exchange"
SYMBOL = "BTCUSD"
MAX_HISTORY = 60       # Number of 1-second candles/ticks to render
REFRESH_RATE_MS = 1000  # Refresh interval
...
def on_message(ws, message):
    ...
    if "type" in data and data["type"] == "l2_updates" and "bids" in data:
        with engine_pipeline.lock:
            # Build current snapshot profile matrix
            ...
            if engine_pipeline.order_book["bids"] and engine_pipeline.order_book["asks"]:
                ...
                # Simulated high frequency candlestick structures 
...
# Expose the underlying Flask server container for Gunicorn cloud deployment
server = app.server
...
def refresh_tradingview_terminal(n):
    ...
    if not times:
        return "HANDSHAKING...", {"color": "#db8c02"}, go.Figure().update_layout(template="plotly_dark")

    last_p = cl[-1]
    ticker_color = "#089981" if ofi_steps_list[-1] >= 0 else "#f23645"
    ticker_text = f"LAST: ${last_p:,.1f} | CUMULATIVE OFI: {current_ofi:+,.1f}"

    # Build TradingView Multi-Plot Grid Dashboard (Price Matrix + Indicator Panel)
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, 
        vertical_spacing=0.05, row_heights=[0.75, 0.25]
    )

    # Panel 1 Trace A: Candle Layout Elements
    fig.add_trace(go.Candlestick(
        x=times, open=op, high=hi, low=lo, close=cl,
        increasing_line_color='#089981', decreasing_line_color='#f23645',
        increasing_fillcolor='#089981', decreasing_fillcolor='#f23645',
        name="Price Action", id="candles"
    ), row=1, col=1)

    # Panel 1 Trace B: Order Book Depth Cluster Overlay (Heatmap Sidebar mapping)
    if bids and asks:
    ...
    # Panel 2 Trace C: OFI Pressure Volume Histogram
    colors = ['#089981' if val >= 0 else '#f23645' for val in ofi_steps_list]
    fig.add_trace(go.Bar(
        x=times, y=ofi_steps_list,
        marker_color=colors, name="OFI Volume Pressure Matrix"
    ), row=2, col=1)

    # Apply TradingView Global Color Scheme Styles
    fig.update_layout(
    ...
    # Format Axes Boundaries & Gridlines
